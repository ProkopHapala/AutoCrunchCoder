...
def _dumps(value, **_kwargs) -> str:
    return _json.dumps(to_serializable(value), ensure_ascii=False)
...
def _constraints(value) -> dict:
    ...
    if not isinstance(parsed, dict): raise ValueError("constraints must be a JSON object")
    return parsed
...
def search_papers(
    ...
    return _dumps(results, default=str)

@mcp.tool()
def find_methods(problem: str, constraints: Optional[dict] = None, limit: int = 20) -> str:
    """Find evidence-bearing method cards for a scientific problem."""
    db = _get_db()
    filters = _constraints(constraints)
    results = db.search(problem, required_tags=filters.get("required_tags"), preferred_tags=filters.get("preferred_tags"),
                        excluded_tags=filters.get("excluded_tags"), year_range=filters.get("year_range"), limit=limit, explain=True)
    enriched = []
    for r in (results if isinstance(results, list) else []):
        pk = r.get("paper_key", r.get("id"))
        if pk: r["methods"] = db.get_methods(pk)
        enriched.append(r)
    return _dumps(enriched, default=str)
...
def find_equations(concept: str, variables: Optional[list[str]] = None, tags: Optional[list[str]] = None, limit: int = 30) -> str:
    ...
    for r in (results if isinstance(results, list) else []):
        pk = r.get("paper_key", r.get("id"))
        equations = []
        for equation in to_serializable(db.get_equations(pk)) if pk else []:
            equation["variables"] = to_serializable(db.get_equation_variables(equation["id"])) if equation.get("id") else []
            symbols = {str(variable.get("symbol", "")).lower() for variable in equation["variables"]}
            if not variables or all(variable.lower() in symbols for variable in variables): equations.append(equation)
        r["equations"] = equations
        enriched.append(r)
    return _dumps(enriched, default=str)

@mcp.tool()
def compare_methods(problem: str, comparison_axes: list[str], constraints: Optional[dict] = None, max_papers: int = 20) -> str:
    ...
    db = _get_db()
    result = db.compare_methods(problem, comparison_axes, constraints=_constraints(constraints), max_papers=max_papers)
    return _dumps(result, default=str)

@mcp.tool()
def build_topic_review(topic: str, focus: Optional[str] = None, constraints: Optional[dict] = None, max_papers: int = 30) -> str:
    """Multi-step: interpret query → find papers → retrieve method cards → compare → synthesize. Returns evidence-backed review."""
    db = _get_db()
    result = db.build_topic_review(topic, focus=focus, constraints=_constraints(constraints), max_papers=max_papers)
    return _dumps(result, default=str)
...
def get_paper(paper_id_or_key_or_doi: str) -> str:
    ...
    db = _get_db()
    result = db.describe_paper(paper_id_or_key_or_doi)
    return _dumps(result, default=str)
...
def explain_paper_match(paper_id: str, query: str) -> str:
    ...
    for r in (results if isinstance(results, list) else []):
        if str(r.get("paper_key", r.get("id"))) == str(paper_id):
            return _dumps(r, default=str)
    return _dumps({"paper_id": paper_id, "match": "not found in search results"})
...
@mcp.tool()
def ingest_pdf(path_or_url: str, tags: Optional[list[str]] = None, dest_dir: Optional[str] = None) -> str:
    """Add a paper from local path or URL (arXiv, DOI, etc.). Requires --allow-mutations."""
    _check_mutations()
    db = _get_db()
    result = db.add_paper(path_or_url, dest_dir=dest_dir)
    if tags: db.add_user_tags(result, tags)
    return _dumps({"result": result})
...
def reprocess_document(paper_id: str, operations: Optional[list[str]] = None) -> str:
    ...
    _check_mutations()
    db = _get_db()
    result = db.ingest_paper(paper_id, operations=operations)
    return _dumps({"result": result}, default=str)
...
def merge_tags(canonical: str, alias: str) -> str:
    ...
    _check_mutations()
    db = _get_db()
    result = db.merge_tags(canonical, alias)
    return _dumps({"result": result}, default=str)
...
def resource_paper(paper_key: str) -> str:
    ...
    db = _get_db()
    return _dumps(db.describe_paper(paper_key))
...
def run_server(transport: str = "stdio", port: int = 8000, allow_mutations: bool = False, db_path: str = None, data_dir: str = None):
    ...
    global _allow_mutations, _db
    _allow_mutations = allow_mutations
    if transport not in ("stdio", "sse"): raise ValueError("transport must be 'stdio' or 'sse'")
    from paperdb import PaperDB
    _db = PaperDB(data_dir=data_dir, db_path=db_path)
