...
async def main():
    ...
    try:
        ...
        async with Client(transport) as client:
            ...
            # Call all listed resources
            for resource in resources:
                print(f"\nCalling resource: {resource.name}")
                res_contents = await client.read_resource(resource.name)
                for c in res_contents:
                    print(f"Response Content: {getattr(c, 'text', c)}")

            # Call selected tools with sample params
            for tool in tools_list:
                print(f"\nCalling tool: {tool.name}")
                # Provide sample parameters per tool
                if tool.name == "calculate_molecular_weight":
                    params = {"formula": "H2O"}
                else:
                    print(f"Skipping tool {tool.name} (no sample params)")
                    continue
                tool_result = await client.call_tool(tool.name, params)
